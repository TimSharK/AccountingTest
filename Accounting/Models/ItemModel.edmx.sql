...
-- --------------------------------------------------
-- Date Created: 07/15/2014 18:33:36
-- Generated from EDMX file: G:\WorkBook\Accounting\Accounting\Models\ItemModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_ParameterCurrentParametersSchema]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[CurrentParametersSchema] DROP CONSTRAINT [FK_ParameterCurrentParametersSchema];
GO
...
GO
IF OBJECT_ID(N'[dbo].[CurrentParametersSchema]', 'U') IS NOT NULL
    DROP TABLE [dbo].[CurrentParametersSchema];
GO
...
-- Creating table 'CurrentParametersSchema'
CREATE TABLE [dbo].[CurrentParametersSchema] (
    [Id] bigint IDENTITY(1,1) NOT NULL,
    [ParameterId] bigint  NOT NULL
);
GO
...
-- Creating primary key on [Id] in table 'CurrentParametersSchema'
ALTER TABLE [dbo].[CurrentParametersSchema]
ADD CONSTRAINT [PK_CurrentParametersSchema]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [ParameterId] in table 'CurrentParametersSchema'
ALTER TABLE [dbo].[CurrentParametersSchema]
ADD CONSTRAINT [FK_ParameterCurrentParametersSchema]
    FOREIGN KEY ([ParameterId])
    REFERENCES [dbo].[Parameters]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_ParameterCurrentParametersSchema'
CREATE INDEX [IX_FK_ParameterCurrentParametersSchema]
ON [dbo].[CurrentParametersSchema]
    ([ParameterId]);
GO
